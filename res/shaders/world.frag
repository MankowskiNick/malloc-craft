...
void main() {
    ...
    if (underwater == 1.0 && y <= waterLevel + 1.0 - waterOffset) {
        vec2 causticCoord = vec2(texCoord.x + sin(time * 0.1), texCoord.y + cos(time * 0.1));

        FragColor = mix(
            texture(caustic, causticCoord) + texture(atlas, coord), 
            vec4(0.0, 0.0, 1.0, 1.0), 
            clamp(dist / waterDistance, 0.3, 0.9));
    }
    else {
