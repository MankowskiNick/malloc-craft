...
float WORLDGEN_BLOCKHEIGHT_MODIFIER = 100.0f;  // Increased for massive mountains
int WORLDGEN_BASE_TERRAIN_HEIGHT = 64; // The height of the base terrain
int WORLDGEN_WATER_LEVEL = 50; // Lowered for larger oceans
float WORLDGEN_BLOCKHEIGHT_FREQUENCY = 0.12f;  // Decreased for larger terrain features
float WORLDGEN_BLOCKHEIGHT_AMPLITUDE = 3.0f;   // Increased for more variation
int WORLDGEN_BLOCKHEIGHT_OCTAVES = 8;          // More octaves for better detail
...
void parse_chunks_settings(json_object chunks_obj) {
    ...
    if (foliage_render_distance.type == JSON_NUMBER) {
        FOLIAGE_RENDER_DISTANCE = (int)foliage_render_distance.value.number;
    }
